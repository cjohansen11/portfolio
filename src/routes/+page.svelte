<script lang="ts">
</script>

<div class="flex h-screen flex-col">
	<div class="m-auto text-center">
		<h1 class="md:text-7xl sm:text-6xl xs:text-4xl">Christian Johansen</h1>
		<h6 class="md:text-5xl sm:text-4xl xs:text-2xl">Full Stack Developer</h6>
		<p class="lg:text-lg md:text-base sm:text-sm xs:text-xs">Website coming soon</p>
		<div class="lg:mt-24 mt-12 xs:mt-8 flex justify-center gap-8 xs:gap-4">
			<a href="https://www.github.com/cjohansen11" target="_blank" rel="noreferrer">
				<img
					src="/icons/icon_github.svg"
					alt="github icon"
					class="m-0 lg:h-12 lg:w-12 md:h-8 md:w-8 sm:h-6 sm:w-6 xs:h-6 xs:w-6"
				/>
			</a>
			<a href="https://www.linkedin.com/in/christian-johansen11" target="_blank" rel="noreferrer">
				<img
					src="/icons/icon_linkedin.svg"
					alt="linkedin icon"
					class="m-0 lg:h-12 lg:w-12 md:h-8 md:w-8 sm:h-6 sm:w-6 xs:h-6 xs:w-6"
				/>
			</a>
			<a href="mailto: cjohansen11@me.com">
				<img
					src="/icons/icon_email.svg"
					alt="email icon"
					class="m-0 lg:h-12 lg:w-12 md:h-8 md:w-8 sm:h-6 sm:w-6 xs:h-6 xs:w-6"
				/>
			</a>
			<a href="/CJohansen_Resume.pdf" target="_blank" rel="noreferrer">
				<img
					src="/icons/icon_download.svg"
					alt="download icon"
					class="m-0 lg:h-12 lg:w-12 md:h-8 md:w-8 sm:h-6 sm:w-6 xs:h-6 xs:w-6"
				/>
			</a>
		</div>
	</div>
</div>
